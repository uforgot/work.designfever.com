...
<?
	...
	$searchSQL = " WHERE B.P_PRS_ID = '$prs_id' AND A.USE_YN = 'Y' AND CONVERT(char(10),A.REG_DATE,120) BETWEEN '$fr_date' AND '$to_date'";
	
	if ($p_category != "")
	{
		if ($p_category == "품의서")
		{
			$searchSQL .= " AND A.FORM_CATEGORY IN ('비용품의서(v2)','비용품의서','프로젝트 관련품의서')";
		}
		else
		{
	...
	$sql = "SELECT COUNT(DISTINCT A.DOC_NO) FROM DF_APPROVAL A WITH(NOLOCK) INNER JOIN DF_APPROVAL_PARTNER B WITH(NOLOCK) ON A.DOC_NO = B.DOC_NO". $searchSQL;
	...
	$sql = "SELECT 
				T.DOC_NO, T.FORM_CATEGORY, T.COUNT, T.P_READ_YN
			FROM 
			(
				SELECT
					ROW_NUMBER() OVER(ORDER BY A.DOC_NO DESC) AS ROWNUM,
					A.DOC_NO, A.FORM_CATEGORY, COUNT(A.SEQNO) AS COUNT, B.P_READ_YN
				FROM 
					DF_APPROVAL A WITH(NOLOCK) INNER JOIN DF_APPROVAL_PARTNER B WITH(NOLOCK)
				ON 
					A.DOC_NO = B.DOC_NO
				$searchSQL
				GROUP BY 
					A.DOC_NO, A.FORM_CATEGORY, B.P_READ_YN
			) T
			WHERE
