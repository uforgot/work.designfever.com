...
<?
	$type = isset($_REQUEST['type']) ? $_REQUEST['type'] : "rewrite";
...
<?
	...
	$sql = "SELECT 
				...
				CONVERT(char(10),REG_DATE,120) AS REG_DATE, CONVERT(char(10),START_DATE,120) AS START_DATE, CONVERT(char(10),END_DATE,120) AS END_DATE 
			FROM 
				DF_APPROVAL WITH(NOLOCK) 
			WHERE 
				DOC_NO = '$doc_no'";
...
<script type="text/JavaScript">
	...
	<? if ($form_category == "비용품의서(v2)") { ?>
		...
		$(".btn_delete").on("click", function() {
			...
			if (flg == "Y") {
				$("input[name=type_"+no+"]:input[value=1]").prop("checked", true);
			}
	...
	function funWrite(type)
	{
		...
		if (type == "save") {
			var type_text = "임시저장";
			var goUrl = "approval_rewrite_act.php";
		}
		if (type == "write") {
			var type_text = "등록";
			var goUrl = "approval_rewrite_act.php";
				...
				<div class="left-wrap">
					<div class="menu">
						<ul>
							...
							<li><a href="approval_write.php?doc_form=009"><? if ($doc_form == "009") { ?><strong class="orange">+ 입사승인계</strong><? } else { ?>+ 입사승인계<? } ?></a></li>
							<li><a href="approval_write.php?doc_form=002"><? if ($doc_form == "002") { ?><strong class="orange">+ 비용품의서</strong><? } else { ?>+ 비용품의서<? } ?></a></li>
					...
					<table class="content-table" width="100%">
						...
						<tbody>
						   <tr>
								...
								<td><?=$new_doc_no?><input type="hidden" name="doc_no" value="<?=$new_doc_no?>"></td>
											...
											<td colspan="3" style="padding-left:1px;">
												...
												<table width="100%" id="paytype_B_<?=$expense?>" <?=$display['B']?>>
													...
													<tr>
														...
														<td>계좌번호</td>
														<td><input type="text" style="width:150px;" name="bank_num_<?=$expense?>" maxlength="30" value="<?=$db_bank_num?>"></td>
														<td>예금주</td>
									...
									<tr style="border-bottom:2px solid;">
										...
										<tr class="last">
											...
											<td colspan="3">
												<table width="100%">
													<tr>
														<td width="100" style="background-color:#efefef">결제방법</td>
														<td>
															<span data-no="<?=$i?>">
												...
												<table width="100%" id="paytype_B_<?=$i?>">
													<tr>
														<td width="100" style="background-color:#efefef">계산서</td>
														<td colspan="6">
													...
													<tr>
														...
														<td>계좌번호</td>
														<td><input type="text" style="width:150px;" name="bank_num_<?=$i?>" maxlength="30"></td>
														<td>예금주</td>
														<td><input type="text" style="width:100px;" name="bank_user_<?=$i?>" maxlength="10"></td>
													</tr>
													<tr style="border-bottom:0px;">
														<td style="background-color:#efefef">지급일자</td>
														<td colspan="6">
												...
												<table width="100%" id="paytype_C_<?=$i?>" style="display:none;">
													<tr style="border-bottom:0px;">
														<td width="100" style="background-color:#efefef">결제구분</td>
														<td>
												...
												<table width="100%" id="paytype_A_<?=$i?>" style="display:none;">
													<tr style="border-bottom:0px;">
														<td width="100" style="background-color:#efefef">이체일자</td>
														<td>
												...
												<table width="100%" id="paytype_H_<?=$i?>" style="display:none;">
													<tr style="border-bottom:0px;">
														<td width="100" style="background-color:#efefef">수령</td>
														<td>경영지원팀 확인 후 직접 수령</td>
													</tr>
												</table>
											</td>
						...
						<?
							...
							while ($record = sqlsrv_fetch_array($rs)) {
								...
								if ($db_type == "A") {
									...
									$db_salary_h  = trim($record['DATA5']);					// 시급
									$db_salary_m  = trim($record['DATA6']);					// 월급
						...
						?>
									...
									<tr style="border-bottom:2px solid;">
										...
										<tr>
											...
											<td colspan="3">
												시급 <input type="text" name="salary_h_<?=$expansion?>" maxlength="5" style="width:50px;" value="<?=$db_salary_h?>">
												월급 <input type="text" name="salary_m_<?=$expansion?>" maxlength="5" style="width:50px;" value="<?=$db_salary_m?>">
											</td>
										</tr>
										<tr>
									...
									<tr style="border-bottom:2px solid;">
										...
										<tr>
											<th class="gray2">급여</th>
											<td colspan="3">
												시급 <input type="text" name="salary_h_<?=$i?>" maxlength="5" style="width:50px;">
												월급 <input type="text" name="salary_m_<?=$i?>" maxlength="5" style="width:50px;">
											</td>
										</tr>
										<tr>
											<th class="gray2">기간</th>
						...
						<div class="attach_section clearfix">
							...
							<div class="right clearfix">
								<div class="clearfix">
									...
									<div id="file_D2" name="file_D2"<? if ($board_file2 == "") { ?> style="display:none;"<? } ?>>
									...
									<div id="file_D3" name="file_D3"<? if ($board_file3 == "") { ?> style="display:none;"<? } ?>>
	...
	?>
			...
			<div class="right section clearfix">
				...
				<div id="partner_div" name="partner_div" class="middle-1" style="height:352px;">
					<input type="hidden" name="total_partner" id="total_partner" value="0">
