...
<?
	...
	$searchSQL = " WHERE PRS_ID = '$prs_id' AND STATUS IN ('전결','결재') AND USE_YN = 'Y' AND CONVERT(char(10),REG_DATE,120) BETWEEN '$fr_date' AND '$to_date'";
	if ($p_category != "")
	{
		if ($p_category == "품의서")
		{
...
<?
			...
			else
			{
				$sql1 = "SELECT TOP 1
							TITLE, CONVERT(char(10),APPROVAL_DATE,102) AS APPROVAL_DATE, PRS_TEAM, PRS_POSITION, PRS_NAME, STATUS, FORM_CATEGORY, FORM_TITLE, 
							(SELECT ISNULL(COUNT(R_SEQNO),0) FROM DF_APPROVAL_REPLY WITH(NOLOCK) WHERE DOC_NO = '$doc_no') AS REPLY, PAYMENT_YN 
						FROM 
							DF_APPROVAL WITH(NOLOCK)
						WHERE
							DOC_NO = '$doc_no'
...
?>
							<tr>
								...
								<td><?=$status?></td>
								<td><?=$reply?> 개</td>
								<td><? if ($payment_yn == "지급") { ?><img src="/img/state_okPay.gif" alt=""><? } ?>
								</td>
								<td class="last">
									<a href="javascript:funReWrite('<?=$doc_no?>');"><img src="/img/btn_reWrittenReport.gif" alt="재상신"></a>
								</td>
