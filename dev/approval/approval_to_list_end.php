...
<?
	...
	$searchSQL = " WHERE A.PRS_ID != '$prs_id' AND B.A_PRS_ID = '$prs_id' AND B.A_STATUS NOT IN ('미결재') AND A.USE_YN = 'Y' AND CONVERT(char(10),A.REG_DATE,120) BETWEEN '$fr_date' AND '$to_date'";
...
?>
							<tr>
								<td colspan="11" class="bold">해당 정보가 없습니다.</td>
							</tr>
<?
	}
	else
	{
		while ($record = sqlsrv_fetch_array($rs))
		{
			...
			{
				$sql1 = "SELECT
							A.TITLE, CONVERT(char(10),A.APPROVAL_DATE,102) AS APPROVAL_DATE, A.PRS_TEAM, A.PRS_POSITION, A.PRS_NAME, A.STATUS, A.PAYMENT_YN, 
							A.FORM_CATEGORY, A.FORM_TITLE, CONVERT(char(10),B.A_REG_DATE,102) AS A_REG_DATE, B.A_STATUS, B.A_ORDER, 
...
?>
							<tr>
								...
								<td>
								<? 
									if ($form_category == "비용품의서" || $form_category == "프로젝트 관련품의서" || $form_category == "비용품의서(v2)") { 
										if ($payment_yn == "지급") { 
								?>
