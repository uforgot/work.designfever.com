...
<script type="text/javascript">
	...
	function mod_Reply(replyno){	//댓글 수정 (댓글번호)
		var frm = document.form;
	...
	function mod_Reply2(replyno,r_replyno){//댓글의 댓글 수정 (댓글번호, 댓글의댓글번호)
		var frm = document.form;
...
<input type="hidden" name="writer_id" value="<?=$board_id?>">	<!-- 글작성자 prs_id -->
	<? include INC_PATH."/top_menu.php"; ?>
		...
		<div class="inner-home">
			...
			<? if ($board == "default") { ?>
			<a href="board_list.php?board=default"><strong>+  게시판</strong></a>
			<a href="board_list.php?board=intranet">+ 인트라넷 문의</a></p>
			<? } else if ($board == "intranet") { ?>
			<a href="board_list.php?board=default">+  게시판</a>
			<a href="board_list.php?board=intranet"><strong>+ 인트라넷 문의</strong></a></p>
			<? } ?>
			<div class="work_wrap clearfix">
				<div class="work_stats_search clearfix">
					...
					<? if ($board_login == $prs_login) {	//작성자는 수정,삭제 가능?>
						<a href="javascript:goDel()" class="btn_right"><img src="../img/btn_del.gif" alt="글 삭제" /></a> 
						<a href="javascript:goEdit()" class="btn_right btn_nomargin"><img src="../img/btn_modi.gif" alt="글 수정" /></a>
					<? } else if ($prf_id == "4") {			//관리자는 삭제만 가능?>
						<a href="javascript:goDel()" class="btn_right"><img src="../img/btn_del.gif" alt="글 삭제" /></a> 
					<? } ?>
				</div>
				<div class="board_view">
					<div class="view_head clearfix">
						...
						<div class="fr">
							<div>등록일 : <strong><span><?=$board_date?></span></strong></div>
								
							<div>조회수 : <strong><?=$board_hit?></strong></div>
							<div class="noline">작성자 : <span><?=$board_team?></span>&nbsp;&nbsp;&nbsp;<strong><?=$board_position?>&nbsp;<?=$board_name?></strong></div>
						</div>
					</div>
					<div class="view_body">
						<div class="view_text">							
							...
							<ul>
								...
								?>
									...
									<div class="c_wrap" style="border-top:1px solid #b2b2b2">
										...
										<div class="c_text"><?=str_replace("\r\n","<br>",$reply_r_contents);?> <a href="javascript:view_reply(<?=$reply_no?>)" class="w_re">+ 댓글쓰기</a></div>
										<div class="c_date">
								...
								?>
										...
										<div id="add_comment_<?=$reply_no?>" name="add_comment_<?=$reply_no?>" class="c_add_comment" style="display:none"> 
											...
											<div class="c_add_btn"><a href="javascript:writeReply2('<?=$reply_no?>','<?=$reply_id?>')">등록</a></div>
										</div>
								<?
									}
								?>
