...
<?
		...
		else
		{
			...
			$exist_flag = 0;
			if (file_exists(BOARD_DIR.$type."/".$Upload_Name.".".$Upload_Type))		//파일 존재여부 체크
			{
				$i = 1;
				while ($exist_flag != 1)
				{
					if (!file_exists(BOARD_DIR.$type."/".$Upload_Name."_".$i.".".$Upload_Type))
					{
						$exist_flag = 1;
			...
			$save_url = BOARD_URL.$type."/".$Upload_Real;
			$save_dir = BOARD_DIR.$type."/".$Upload_Real;
