...
<?
	...
	$retUrl = "contact_list.php?page=". $page ."&board=". $board;
	if ($subject != "")
	{
...
<?
	...
	if (sqlsrv_has_rows($rs) > 0)
	{
		...
		$book_tmp4 = trim($record['TMP4']);
		$book_tmp5 = trim($record['TMP5']);
		$book_tmp6 = trim($record['TMP6']);
		$book_tmp7 = trim($record['TMP7']);
		$book_tmp8 = trim($record['TMP8']);
		$book_tmp9 = trim($record['TMP9']);
	}
	else
	{
?>
...
<script type="text/javascript">
	...
	function writeReply(){ //댓글 달기
		...
		frm.action = 'contact_reply_act.php';
		frm.submit();
	}; 
	...
	function writeReply2(replyno,replyid){ //댓글의 댓글 달기  
		...
		var frm = document.form;
			...
			frm.action = 'contact_reply_act.php?reply_no='+replyno+'&reply_id='+replyid+'&reply_contents2='+reply_contents2+'&keyfield='+frm.keyfield.value+'&keyword='+frm.keyword.value+'&page='+frm.page.value+'&seqno='+frm.seqno.value;
			frm.submit();
	}
	...
	function modifyReply(replyno){ 
		...
		frm.action = 'contact_reply_act.php?reply_no='+replyno;
		frm.submit();
	}
	...
	function modifyReply2(replyno,r_replyno){ 
		...
		frm.modify_contents.value = modify_contents.value;
		frm.action = 'contact_reply_act.php?reply_no='+replyno+'&r_reply_no='+r_replyno;
		frm.submit();
	}
        ...
        <div class="container">
            <nav class="level is-mobile">
                ...
                <div class="level-right">
                    <? if ($book_id == $prs_id || $prf_id == "4") {	?><!--작성자와 관리자 수정,삭제 가능-->
                    <p class="buttons">
            ...
            <div class="content">
                ...
                <h1>
                    <? if (($book_tmp1 != "") && ($board == "ilab")) { ?>
                        [<?=$book_tmp1?>] <?=$book_title?>
                    <? } else { ?>
                        <?=$book_title?>
                    <? } ?>
                    <? if ($book_depth != "0") { ?> <span class="tag is-rounded td-tag"><?=$book_depth?></span> <? } ?>
                    <? if ($book_file1 != "" || $book_file2 != "" || $book_file3 != "") { ?> &nbsp;<span class="icon is-small td-icon"><i class="fas fa-file"></i></span> <? } ?>
                </h1>
                <input type="hidden" name ="rep_depth" value="<?=$book_depth?>">	<!-- 댓글개수 -->

                <p class="is-size-7">작성자
                    <?=$book_team?> <?=$book_position?> <?=$book_name?>
                    &nbsp;&nbsp;|&nbsp;&nbsp;
                    조회수 <?=$book_hit?>
                    &nbsp;&nbsp;|&nbsp;&nbsp;
                    등록일  <?=$book_date?>
            ...
            <? if ($book_file1 != "" || $book_file2 != "" || $book_file3 != "") { ?>
                <nav class="panel">
                    ...
                    <? if ($book_file1 != "") { ?>
                        <a class="panel-block" href="javascript:file_download('book',1,<?=$seqno?>);">
                <span class="panel-icon">
                  <i class="fas fa-file" aria-hidden="true"></i>
                    ...
                    <? } ?>
                    <? if ($book_file2 != "") { ?>
                        <a class="panel-block" href="javascript:file_download('book',2,<?=$seqno?>);">
                <span class="panel-icon">
                  <i class="fas fa-file" aria-hidden="true"></i>
                </span>
                            <?=$book_file2?>
                    ...
                    <? } ?>
                    <? if ($book_file3 != "") { ?>
                        <a class="panel-block" href="javascript:file_download('book',3,<?=$seqno?>);">
                <span class="panel-icon">
                  <i class="fas fa-file" aria-hidden="true"></i>
                </span>
                            <?=$book_file3?>
            ...
            <article class="media" id="modify_c_reply_<?=$reply_no?>" style="display:none;">
                ...
                <div class="media-content">
                    ...
                    <div class="field">
                        <p class="control">
                            <a id="reply_btn_<?=$reply_no?>" href="javascript:modifyReply('<?=$reply_no?>')">수정</a>
                        </p>
                    </div>
                </div>
