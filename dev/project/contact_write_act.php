...
<?
	...
	$tmp9 = isset($_REQUEST['tmp9']) ? $_REQUEST['tmp9'] : null;
...
<?
	...
	if ($type == "write")
	{
		...
		$sql = "INSERT INTO DF_BOARD
				(SEQNO, PRS_ID, PRS_NAME, PRS_LOGIN, PRS_TEAM, PRS_POSITION, TITLE, CONTENTS, HIT, FILE_1, FILE_2, FILE_3, REP_DEPTH, NOTICE_YN, REG_DATE, TMP1, TMP2, TMP3, TMP4, TMP5, TMP6, TMP7, TMP8, TMP9)
				VALUES
				('$seq', '$writer_id', '$writer_name', '$writer', '$writer_team', '$writer_position', '$title', '$contents', '$hit', '$myFile1_Real', '$myFile2_Real', '$myFile3_Real', '$depth', '$notice_yn', getdate(), '$tmp1', '$tmp2', '$board', '$tmp4', '$tmp5', '$tmp6', '$tmp7', '$tmp8', '$tmp9')";
	}
	else if ($type == "modify")
	{
		$type_title = "수정";
		$retUrl = "contact_detail.php?page=". $page ."&board=". $board ."&seqno=". $seqno;
		if ($subject != "")
		{
			$retUrl .= "&subject=". $subject;
		}
	...
	else
	{
		...
		if ($board == "contact" && $type == "write" && $send_mail == "Y") {
			...
			$content.= "<p><b>+ 프로젝트:</b> ".iconv("EUC-KR","UTF-8", $title)."<br></p>
						<p><b>+ 일정:</b> ".iconv("EUC-KR","UTF-8", $tmp9)."<br></p>
						...
						<p><b>+ 의뢰내용:</b><br></p>
						<p><a href='http://work.designfever.com/project/contact_detail.php?board=contact&page=1&keyfield=ALL&keyword=&seqno=".$seq."&type=ret' target='_blank'>자세히 보기..</a><br></p>";
