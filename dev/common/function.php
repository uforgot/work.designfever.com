...
//			return false;
		...
		if ( $block > 1 )
			$result .= "<a class='pagination-previous' href=javascript:Paging(this.form,'$prev_block','$current_url');>이전 10페이지</a>";
		if($block < $total_block)
			$result .= "<a class='pagination-previous' href=javascript:Paging(this.form,'$next_block','$current_url');>다음 10페이지</a>";				
	 		$result .= "<ul class='pagination-list '>";	
		
		if ( $page > 1 )
			$result .= "<a href=javascript:Paging(this.form,'$prev','$current_url'); class='p_prev'></a>";

		for ( $go_page; $go_page <= $last_page; $go_page++ ) {
			if ( $page == $go_page )
				$result .= " <a class='pagination-link is-current' aria-label='Page 46' aria-current='page'>$go_page</a>"; //현재페이지 출력
			else
				$result .= "<a class='pagination-link' href=javascript:Paging(this.form,'$go_page','$current_url');>$go_page</a>";//다음페이지들 출력
		}

		if ( $page < $total_page )
			$result .= "<a href=javascript:Paging(this.form,'$next','$current_url'); class='p_next'></a>";
	...
	function getProfileImg($file)
	{
		if ($file == "")
		{
			if ($name == "no" && $id == false)
			{
				//$result = "<img src='/img/bg_default.png' alt='profile_img' width='". $size ."' height='". $size ."' style='vertical-align:middle;' name='". $name ."' value='". $id ."' />";
				$result = "<img src='/img/bg_default.png' alt='profile_img'/>";
			}
			else
			{
				//$result = "<img src='/img/noimg.jpg' alt='profile_img' width='". $size ."' height='". $size ."' style='border:1px solid #000;' name='". $name ."' value='". $id ."' />";
				$result = "<img src='/img/noimg.jpg' alt='profile_img' />";
			}
		}
		else
		{
			//$result = "<img src='/file/". $file ."' alt='profile_img' width='". $size ."' height='". $size ."' style='vertical-align:middle;' name='". $name ."' value='". $id ."' />";
			$result = "<img src='/file/". $file ."' alt='profile_img'/>";
		}
		return $result;
	}
	...
	{ // $week 0:일 ~ 6:토 
		...
		$e_date = date("Y-m-d", strtotime("$s_date +4 day"));
	...
	*/
	...
	function getWeeklyBtn() {
		...
		if($week == 5) {
			...
			$result = "<a class='button is-fullwidth is-danger' href='javascript:go_weekly();' title='주간보고서작성'>주간보고서작성</a>";			
	...
	function getWeeklyBtn_M() {
		...
		if($week == 5) {
			//$result = "<a href='javascript:go_weekly();' title='주간보고서작성'><img src='img/btn_weekly.png' alt='주간보고서작성' /></a>";
			//$result = "<a class='button is-fullwidth is-danger' href='javascript:go_weekly();' title='주간보고서작성'>주간보고서작성</a>";			
			$result=" <a class='button is-small' href='javascript:go_weekly();' title='주간보고서작성'>주간보고서작성</a>";
		}
...
#################################################################
	...
	function chkWeekly($cur_date,$week) 
	{
		...
		if ($week == "prev") 
		{
			$ndate = date("Y-m-d", strtotime("$cur_date -3 day"));
		}
		else
		{
			$ndate= date("Y-m-d");
	...
	*/
	...
	//KISA_SHA256 비밀번호 암호화
	function kisa_sha256($str) {
