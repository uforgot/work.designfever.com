...
<?
	...
	$where = " AND PRF_ID IN (1,2,3,4,5,7) AND PRS_ID NOT IN(102)";
...
<?
	$teamSql = "SELECT 
				...
				WHERE 
					TEAM IN ('커뮤니케이션전략기획1실','디자인1실','모션그래픽스1실','비주얼 인터랙션 디벨롭먼트 1실','경영지원팀') 
				ORDER BY 
					SORT";
...
<?
	$teamSql = "SELECT 
				...
				FROM 
					DF_TEAM_CODE WITH(NOLOCK) 
				WHERE 
					TEAM IN ('CCO','CSO','커뮤니케이션전략기획2실','디자인2실 1팀','디자인2실 2팀','모션그래픽스2실','비주얼 인터랙션 디벨롭먼트 2실') 
				ORDER BY 
					SORT";
	$teamRs = sqlsrv_query($dbConn, $teamSql);
	while($teamRecord = sqlsrv_fetch_array($teamRs))
