...
if (REMOTE_IP == "")
{
	...
	//출근은 한번만 체크 가능 
	//퇴근은 복수체크가능 (만약 이미 퇴근체크를 했을경우는 이미퇴근체크한 값을 받아서 업데이트 해준다)
	...
	if (sizeof($record) > 0)
	{
		...
		$message = $col_prs_name ."님, 오늘 하루도 수고하셨습니다!! 퇴근체크가 완료되었습니다.". date("Y-m-d H:i:s");
	}
	else
	{
		echo "등록되지 않은 카드입니다.";
	...
	if (sizeof($record) > 0)
	{
		...
		$yesterday_checktime1 = $record['YESTERDAY_CHECKTIME1'];	//어제 출근시간	
		$yesterday_checktime2 = $record['YESTERDAY_CHECKTIME2'];	//어제 퇴근시간
		$yesterday_totaltime = $record['YESTERDAY_TOTALTIME'];		//어제 근무시간
		$yesterday_overtime = $record['YESTERDAY_OVERTIME'];		//어제 연장근무시간
		$yesterday_memo1 = $record['YESTERDAY_MEMO1'];				//어제 출근 정보
		$yesterday_memo2 = $record['YESTERDAY_MEMO2'];				//어제 퇴근 정보
		$today_gubun1 = $record['TODAY_GUBUN1'];					//오늘 출근
		$today_gubun2 = $record['TODAY_GUBUN2'];					//오늘 퇴근
		$today_checktime1 = $record['TODAY_CHECKTIME1'];			//오늘 출근시간	
		$today_checktime2 = $record['TODAY_CHECKTIME2'];			//오늘 퇴근시간
		$today_totaltime = $record['TODAY_TOTALTIME'];				//오늘 근무시간
		$today_memo1 = $record['TODAY_MEMO1'];						//오늘 출근 정보
		$today_memo2 = $record['TODAY_MEMO2'];						//오늘 퇴근 정보
		...
		$next_checktime1 = $record['NEXT_CHECKTIME1'];				//내일 출근시간	
		$next_checktime2 = $record['NEXT_CHECKTIME2'];				//내일 퇴근시간
	...
	else
	{
		...
		echo "퇴근 체크 오류(Error". $error_no .")입니다. 번호를 기억하셔서 개발팀에 문의해 주세요";
		exit;
	}

	if ($today_checktime1 == "" && $yesterday_checktime1 == "")			//오늘 출근체크 X, 어제 출근체크 X
	{
		echo $col_prs_name ."님, 출근체크를 하지 않으셨습니다.";
		exit;
	}
	else
	{
...
//				WHERE PRS_ID = '$col_prs_id' AND DATE = '$yesterday'";
	...
	{
		echo "오늘 출근체크를 하지 않으셨습니다. 근태수정요청 게시판에 출퇴근 정보를 알려주세요.";
		exit;
	}
