...
<?
	...
	{
		$teamSQL = " AND PRS_TEAM IN ('커뮤니케이션전략기획1실','디자인1실','모션그래픽스1실','인터랙티브테크놀로지1실')";
	}
	...
	$sql = "SELECT 
			...
			ORDER BY 
				CASE 
					...
					WHEN PRS_TEAM = '인터랙티브테크놀로지1실' THEN 4 END, CASE ". $orderby. " END, PRS_JOIN, PRS_NAME";
...
<!--
	...
	{
		f.target="_self";
		f.action="commuting_member2.php";
		f.page.value = "1";
		f.submit();
	}
	...
	<? } else { ?>
		...
		frm.target="_self";
		frm.action="commuting_member2.php";
