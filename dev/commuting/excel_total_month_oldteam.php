...
?>
	...
	<table border=1>
		...
		<tbody>
<?
		$teamList = "'dm1'##'dm2'##'digital marketing division'##'dx1'##'dx2'##'digital experience division'##'brand experience team'##'design1'##'design2'##'design1 division'##'design3'##'design4'##'design5'##'design2 division'##'fc'##'film & content division'##'mg1'##'mg2'##'motion graphic division'##'ix1'##'ix2'##'df lab'##'ixd'##'경영지원팀'";
		$teamList1 = "##dm1##dm2##dx1##dx2##brand experience team##design1##design2##design3##design4##design5##fc##mg1##mg2##ix1##ix2##ixd##경영지원팀";
...
<?
				...
				else
				{
					...
					$div_biz_commute = $record2['BIZ_COMMUTE'];	//평일정상출근
					$div_law_commute = $record2['LAW_COMMUTE'];	//휴일출근
					$div_lateness = $record2['LATENESS'];			//지각
					$div_vacation1 = $record2['VACATION1'];				//연차
					$div_vacation2 = $record2['VACATION2'];				//병가
					$div_vacation3 = $record2['VACATION3'];				//경조사
					$div_refresh = $record2['REFRESH'];					//리프레시휴가
					$div_project = $record2['PROJECT'];					//프로젝트휴가
					$div_nomoney = $record2['NOMONEY'];					//무급휴가
					$div_edu = $record2['EDU'];							//교육/훈련
					$div_etc = $record2['ETC'];							//기타
					$div_commute_day = $record2['COMMUTE_DATE'];			//근무일수
					$div_subvacation1 = $record2['SUBVACATION1'];	//오전반차
					$div_subvacation2 = $record2['SUBVACATION2'];	//오후반차
					$div_avgtime1 = $record2['AVGTIME1'];				//평균출근시
					$div_avgminute1 = $record2['AVGMINUTE1'];			//평균출근분
					$div_avgtime2 = $record2['AVGTIME2'];				//평균퇴근시
					$div_avgminute2 = $record2['AVGMINUTE2'];			//평균퇴근분
					$div_avg_time = $record2['AVG_TIME'];				//평균근무시간시
					$div_avg_minute = $record2['AVG_MINUTE'];			//평균근무시간분
					$div_biz_total_time = $record2['BIZ_TOTAL_TIME'];		//평일총근무시간시
					$div_biz_total_minuate = $record2['BIZ_TOTAL_MINUTE'];	//평일총근무시간분
					$div_total_time = $record2['TOTAL_TIME'];			//총근무시간시
					$div_total_minute = $record2['TOTAL_MINUTE'];		//총근무시간분
					$div_over_time = $record2['OVER_TIME'];				//초과근무시간시 - 하루 9시간 이상 근무한 내역에 대한 월 총합시간
					$div_over_minute = $record2['OVER_MINUTE'];			//초과근무시간분 - 하루 9시간 이상 근무한 내역에 대한 월 총합시간
					$div_over_day = $record2['OVER_DATE'];				//초과일수
					$div_under_time = $record2['UNDER_TIME'];			//미만근무시간시
					$div_under_minute = $record2['UNDER_MINUTE'];		//미만근무시간분
					$div_under_day = $record2['UNDER_DATE'];				//미만일수
					$div_off_time = $record2['OFF_TIME'];				//외출 시
					$div_off_minute = $record2['OFF_MINUTE'];			//외출 분
					$div_biz_off_time = $record2['BIZ_OFF_TIME'];		//평일외출 시
					$div_biz_off_minute = $record2['BIZ_OFF_MINUTE'];	//평일외출 분
					$div_real_over = $record2['REAL_OVER'];				//연장근무시간분단위
					$div_real_avg = $record2['REAL_AVG'];				//평균근무시간분단위
					$div_real_off = $record2['REAL_OFF'];				//평균외출시간분단위
