...
<?
	...
	$p_sort = isset($_POST['sort']) ? $_POST['sort'] : null;

	$rd_seqno = isset($_POST['rd_seqno']) ? $_POST['rd_seqno'] : null;
	...
	$p_answer = isset($_POST['answer']) ? $_POST['answer'] : null;
	...
	{
		// 근태수정 요청 상태 업데이트
		$sql = "UPDATE DF_CHECKTIME_REQUEST SET ANSWER = '$p_answer', STATUS = 'CANCEL', OK_DATE = getdate(), OK_NAME = '$prs_name' WHERE SEQNO = '$rd_seqno'";
...
<?
	...
	{
		// 근태수정 요청 상태 업데이트
		$sql = "UPDATE DF_CHECKTIME_REQUEST SET ANSWER = '$p_answer', BST_FLAG = 'Y', BST_DATE = getdate(), BST_NAME = '$prs_name' WHERE SEQNO = '$rd_seqno'";
		$rs = sqlsrv_query($dbConn,$sql);
?>
		<!--
		<script language="javascript">
...
<?
	...
	else if ($mode == "delete_request")
	{
		$sql = "DELETE FROM DF_CHECKTIME_REQUEST WHERE DATE = '$p_date' AND PRS_ID = '$p_id'";
		$rs = sqlsrv_query($dbConn,$sql);
	}
	else
	{
...
<?
		...
		// 근태수정 요청 상태 업데이트
		$sql = "UPDATE DF_CHECKTIME_REQUEST SET ANSWER = '$p_answer', STATUS = 'OK', OK_DATE = getdate(), OK_NAME = '$prs_name' WHERE SEQNO = '$rd_seqno'";
		$rs = sqlsrv_query($dbConn,$sql);
	}
...
?>
	<script language="javascript">
		...
		if (type == "team")
		{
			...
			top.opener.location.href = "commuting_member_request.php?year="+year+"&month="+month+"&type="+type+"&team="+team+"&page="+page+"&sort="+sort;
		}
		else if (type == "person")
		{
			var name = frm.name.value;
			top.opener.location.href = "commuting_member_request.php?year="+year+"&month="+month+"&type="+type+"&name="+name+"&page="+page+"&sort="+sort;
		}
		else
		{
			top.opener.location.href = "commuting_member_request.php?year="+year+"&month="+month+"&page="+page+"&sort="+sort;
		}
