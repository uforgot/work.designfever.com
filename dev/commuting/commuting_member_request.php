						...
						<tr>
							...
							?>
								...
								<select name="sort" onChange="transSort(this.value)">
									...
									<option value="request" <?if($p_sort=="request"){echo "selected";}?>>근태수정요청</option>
									<option value="request-ing" <?if($p_sort=="request-ing"){echo "selected";}?>>근태수정요청(미처리)</option>
								</select>

								<font style="color:#ef0000">※ 전달 근태수정 미처리 : <b><?=number_format($prev_ing_tot)?></b> 건</font>
							</td>
							<td align="right">
...
?>
						<tr class="line_up">
							...
							<td height="70"><span class="in">출</span></td>
<?
...
?>
						...
						<tr class="line_down">
							<td height="70"><span class="out">퇴</span></td>
<?
			for ($j=0; $j<sizeof($col_date_ex); $j++)
			{
				if ($col_date_ex[$j] != "")
