...
function file_download(menu,file,seq) {
	var agent = navigator.userAgent;

	if (agent.match(/Mobile|Windows CE|Opera Mini|POLARIS|iPad/) != null){
		window.open('/common/download.php?menu='+menu+'&file='+file+"&seq="+seq,'download');
	} else {
		location.href = '/common/download.php?menu='+menu+'&file='+file+"&seq="+seq;
	}
}
...
function changeSVGToGif(){
	if( !Modernizr.svg ) var checkType = ".svg";
	else return;

	var child = $(document).find(".js-svg");
	var tot = child.length;
	for( var i=0; i<tot; i++ ) {
		var url = $(child[i]).attr('src').split(checkType)[0]+'.png';
		$(child[i]).attr( {src:url} );
	}
}