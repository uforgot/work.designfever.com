...
<?
	...
	while ($listRow = sqlsrv_fetch_array($listRs))
	{
		...
		$list .= "<tr>
					<td width=\"5%\" class=\"$class1\">".$i."</td>
					<td width=\"12%\" class=\"$class1\">".$date.' '.$visit_stime."</td>
					<td width=\"12%\" class=\"$class1\">".$visit_company."</td>
					<td width=\"10%\" class=\"$class1\">".$visit_visitor."</td>
					<td width=\"10%\" class=\"$class1\">".$visit_carno."</td>
					<td width=\"10%\" class=\"$class1\">".$visit_phone."</td>
					<td width=\"*\" class=\"$class1\" style=\"text-align:left;\">".$visit_memo." (담당자: ".$visit_name.")</td>
				</tr>\n";
	...
	if($total == 0) {
		$list = "<tr>
					<td width=\"100%\" colspan=\"7\">금일 예약된 방문객이 없습니다.</td>
				</tr>\n";
	}
