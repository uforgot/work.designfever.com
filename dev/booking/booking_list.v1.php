...
<?
	...
	while ($listRow = sqlsrv_fetch_array($listRs))
	{
		...
		for($i=0;$i<$booking_line;$i++) {
			...
			if($i==0) {
				$booking_info = "<a href=\"./booking_write.php?type=modify&date=$date&seqno=$booking_seqno\"><span style='color:#000;font-weight:bold;'>".$booking_stime." ~ ".$booking_etime."</span>&nbsp;(예약자: ".$booking_name.")<br>";
...
<form method="get" name="form">
	<? include INC_PATH."/top_menu.php"; ?>
		...
		<div class="inner-home">
			...
			<a href="../book/book_list.php?board=book">+  회사생활백서</a>
			<a href="../book/book_list.php?board=free">+  자유게시판</a>
			<a href="../book/book_list.php?board=ilab">+  iLab</a>
			<a href="../book/book_list.php?board=club">+  동호회게시판</a>
			<a href="../book/book_list.php?board=edit">+  근태수정요청</a>
			<? if (in_array($prs_id,$happyLab_arr) == true) { ?>
				<a href="book_list.php?board=happy">+  행복연구소</a>
			<? } ?>
			<a href="booking_list.php"><strong>+  회의실사용예약</strong></a>
			</p>
				...
				<table class="notable work5 project_detail board_list" width="100%" style="margin-bottom:10px;">
					...
					<tbody>
						<tr>
							...
							?>
									<tr>
										<td width="20%" class="<?=$room1_style?>" <?=$room1_btn?>>&nbsp;<?=$Data['ROOM1'][$time]['info']?></td>
										<td width="20%" class="<?=$room2_style?>" <?=$room2_btn?>>&nbsp;<?=$Data['ROOM2'][$time]['info']?></td>
										<td width="20%" class="<?=$room3_style?>" <?=$room3_btn?>>&nbsp;<?=$Data['ROOM3'][$time]['info']?></td>
										<td width="20%" class="<?=$room4_style?>" <?=$room4_btn?>>&nbsp;<?=$Data['ROOM4'][$time]['info']?></td>
										<td width="20%" class="<?=$room5_style?>" <?=$room5_btn?>>&nbsp;<?=$Data['ROOM5'][$time]['info']?></td>
									</tr>
							<?
								}
							?>
