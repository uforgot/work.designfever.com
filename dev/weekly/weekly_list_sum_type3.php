...
<?
	...
	if($next_week) $next_link = "<a href='weekly_list_sum_type3.php?week=".$next_week."&team=".$team."'>▶</a>";
	else $next_link = "▶";
	...
	if($prev_week) $prev_link = "<a href='weekly_list_sum_type3.php?week=".$prev_week."&team=".$team."'>◀</a>";
	else $prev_link = "◀";
	...
	$searchSQL = " WHERE SUBSTRING(CAST(WEEK_ORD AS CHAR(7)),1,6) IN('$prev_month','$this_month','$next_month') AND B.THIS_WEEK_RATIO > 0";
	...
	while ($record = sqlsrv_fetch_array($rs))
	{
		...
		// 프로젝트 작업 기간 정보(보고서 완료 기준)
		$searchSQL1 = " WHERE B.PROJECT_NO = '".$record['PROJECT_NO']."' AND B.PRS_ID = '".$record['PRS_ID']."' AND A.COMPLETE_YN = 'Y' AND SUBSTRING(CAST(A.WEEK_ORD AS CHAR(7)),1,6) IN('$prev_month','$this_month','$next_month')";

		$sql1 = "SELECT 
					A.WEEK_ORD, SUBSTRING(A.WEEK_AREA,1,10) start_date, SUBSTRING(A.WEEK_AREA,12,10) end_date, B.THIS_WEEK_RATIO, A.PRS_NAME
				FROM 
					DF_WEEKLY A WITH(NOLOCK) 
					INNER JOIN DF_WEEKLY_DETAIL B WITH(NOLOCK) 
					ON A.SEQNO = B.WEEKLY_NO
		...
		{
			if($record1['THIS_WEEK_RATIO'] > 0) {
				$proj_date[$record['PROJECT_NO']."^".$record['PRS_ID']][] = array
																			(
																				'start_date'=>str_replace('.','-',$record1['start_date']),
																				'end_date'=>str_replace('.','-',$record1['end_date']),
																				'this_ratio'=>$record1['THIS_WEEK_RATIO']
																			);
			}

			$proj_ratio[$record['PROJECT_NO']."^".$record['PRS_ID']][$record1['WEEK_ORD']] = $record1['THIS_WEEK_RATIO'];
		}
	}
...
<script type="text/javascript">
	function weekSearch(val) {
		document.location.href = "./weekly_list_sum_type3.php?week=" + val + "&team=<?=$team?>";
	}
	...
	function teamSearch(val) {
		...
		document.location.href = "./weekly_list_sum_type3.php?week=<?=$week?>&team=" + val;
	}
</script>
</head>
<body>
		...
		foreach($list as $key1 => $val1)
		{
			...
			foreach($val1 as $key2 => $val2)
			{
				...
				for($i=1;$i<=$l1;$i++) {
					...
					$this_month_day .= "<td style='border-bottom:1px solid #e3e3e3;".$color1.$graph1."'>$i</td>";
				}
				...
				for($j=1;$j<=$l2;$j++) {
					...
					$next_month_day .= "<td style='border-bottom:1px solid #e3e3e3;".$color2.$graph2."'>$j</td>";
				}

				if($this_count > 0 || $next_count > 0) {

					// 현재 선택 주차의 참여율
					$this_ratio = $proj_ratio[$key1."^".$val2['id']][$week];
					if(!$this_ratio) $this_ratio = 0;

					if($key1 == "DF0000_ETC") 
					{
						$name .= "<div style='padding:1px 0 2px 1px;'><table cellpadding='0' cellspacing='0' class='name'><tr>";
						$name .= "<td>".$val2['name']." (".$this_ratio."%)</td>";
						$name .= "</tr></table></div>";
					}
					else					  
					{
						$name .= "<div style='padding:1px 0 2px 1px;'><table cellpadding='0' cellspacing='0' class='name'><tr>";
						$name .= "<td>".$val2['part'].".".$val2['name']." (".$this_ratio."%)</td>";
						$name .= "</tr></table></div>";
					}

					$this_month .= "<div style='padding:1px 0 1px 1px;'><table cellpadding='0' cellspacing='0' class='graph'><tr>".$this_month_day."</tr></table></div>";
					$next_month .= "<div style='padding:1px 0 1px 1px;'><table cellpadding='0' cellspacing='0' class='graph'><tr>".$next_month_day."</tr></table></div>";
