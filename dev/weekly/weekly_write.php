...
if (!$seqno || !$record) {
    ...
    $searchSQL = " WHERE B.PRS_ID = '$weekly_prs_id' AND DATEADD(DD,7,A.END_DATE) >= DATEADD(DD,$add,GETDATE()) AND A.USE_YN = 'Y'"; // 정상 작성일인 금요일 기준으로..
...
<form method="post" name="form" action="weekly_write_act.php">
    ...
    <input type="hidden" name="prs_id" value="<?= $prs_id ?>">                <!-- 작성자 prs_id-->
