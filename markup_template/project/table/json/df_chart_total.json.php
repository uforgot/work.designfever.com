										...
										$t_data_lists=array();		
											...
											$record=sqlsrv_fetch_array($rs);	
													...
													$rs = sqlsrv_query($dbConn,$sql);
														...
														{	
															//프로젝트 기본정보 뽑아내기
																...
																$sql2 ="	SELECT COUNT(B.THIS_WEEK_RATIO) AS THIS_WEEK_RATIO 	 	
																				 ...
																				 --, CONVERT(VARCHAR(10), COUNT(THIS_WEEK_RATIO)) +' ('+ CONVERT(VARCHAR(10),SUM(B.THIS_WEEK_RATIO) /convert(float,100) )+')' AS TOT
																				 , CONVERT(VARCHAR(10),SUM(B.THIS_WEEK_RATIO) /convert(float,100)) +' ('+ CONVERT(VARCHAR(10), COUNT(THIS_WEEK_RATIO))+')' AS TOT
																				 , A.PRS_TEAM
																			   FROM DF_WEEKLY A WITH (NOLOCK)
																			  INNER JOIN DF_WEEKLY_DETAIL B WITH (NOLOCK) 
																				 ON A.SEQNO = B.WEEKLY_NO
																	...
																	{				
																		...
																		$col_sum = $record['SUM_RATIO'] / 100;
																		$col_sum_arr = $col_sum_arr . $col_sum ."##";	
																		$col_sum_arr2 = substr($col_sum_arr,0,-2);	
																		
																		$col_tot = $record['TOT'];
																		$col_tot_arr = $col_tot_arr . $col_tot ."##";	
																		$col_tot_arr2 = substr($col_tot_arr,0,-2);	
																		
																																			
																	}																										
																	$col_ratio_ex = explode("##",$col_ratio_arr2);
																	$col_sum_ex = explode("##",$col_sum_arr2);
																	$col_tot_ex = explode("##",$col_tot_arr2);
																	
																	
															$sql3="SELECT COUNT(A.PRS_NAME) AS CNT  
																		--, ISNULL(CONVERT(VARCHAR(10), COUNT(A.PRS_NAME)) +' ('+ CONVERT(VARCHAR(10),SUM(B.THIS_WEEK_RATIO) /convert(float,100) )+')','0') AS TOT		
																		, ISNULL(CONVERT(VARCHAR(10),SUM(B.THIS_WEEK_RATIO) /convert(float,100))+' ('+ CONVERT(VARCHAR(10), COUNT(THIS_WEEK_RATIO))+')','0') AS TOT																																	
																	 FROM DF_WEEKLY A WITH (NOLOCK)
																	INNER JOIN DF_WEEKLY_DETAIL B WITH (NOLOCK) 
																	   ON A.SEQNO = B.WEEKLY_NO  	
																	WHERE B.PROJECT_NO ='$project_no'
														...
														array_push($t_data_lists, array("project_name"=>$project_name, "pm"=>$pm, "start_time"=>$start_time, "end_time"=>$end_time, "to"=>$to, "points"=>$col_tot_ex));												
														}	
...
//////////////////////////////////////////// 직원별 세로열  table_data ////////////////////////////////////////////
										...
										$table_data_lists=array();						
													...
													$rs = sqlsrv_query($dbConn,$sql);
														...
														array_push($table_data_lists, array("project_name"=>$project_name, "pm"=>$pm, "start_time"=>$start_time, "end_time"=>$end_time, "to"=>$to, "points"=>$col_ratio_ex));												
														}																																			
...
//TOTAL RESULT
	...
	file_put_contents("df_chart_total_".$last_week.".json", urldecode($data));
